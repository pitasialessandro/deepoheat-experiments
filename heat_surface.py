...
if __name__ == '__main__':
    ...
    parser.add_argument('--decay_steps', type=int, default=500, help='steps between each lr decay (x0.9)')
    parser.add_argument('--train_data', type=str, default='data/fs_train_surface.npy', help='training power maps (.npy)')
    parser.add_argument('--tag', type=str, default='', help='suffix appended to the result directory')
    ...
    # set up the device
    fs_train = jnp.load(args.train_data).reshape(-1,21**2)
    ...
    result_dir = os.path.join(root_dir, 'nf'+str(args.batch)+'_nc'+str(args.nc) + '_branch_' + str(args.branch_depth) + 
                              ...
                              '_'+str(args.trunk_hidden)+'_r'+ str(args.r)) + args.tag
    ...
    schedule = optax.exponential_decay(args.lr, args.decay_steps, 0.9)
    ...
    # solo array float: deve combaciare con la struttura dei gradienti di filter_value_and_grad
